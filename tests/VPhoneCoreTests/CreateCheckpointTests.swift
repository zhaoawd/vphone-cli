    ...
    @unchecked Sendable
{
    ...
    private var _treeSeen: [VPhoneCreateStage: String] = [:]
    ...
    /// Per executed stage: the restore tree's checkpoint availability and whether it existed on disk.
    var treeSeen: [VPhoneCreateStage: String] { lock.withLock { _treeSeen } }
    ...
    func execute(_ stage: VPhoneCreateStage, context: VPhoneCreateStageContext) throws -> [String: String] {
        let tree = context.checkpoint.artifact("restore_tree")?.availability.rawValue ?? "unrecorded"
        let onDisk = FileManager.default.fileExists(atPath: context.bundleURL.appendingPathComponent("Restore").path)
        lock.withLock { _executed.append(stage); _treeSeen[stage] = "\(tree), \(onDisk ? "present" : "absent")" }
        onExecute?(stage)
    ...
    func artifactsRewrittenOnRerun(_ stage: VPhoneCreateStage) -> Set<String> {
        if stage == .prepare { return ["restore_tree"] }
        return Self.diskStages.contains(stage) ? ["disk"] : []
    }
...
@Suite struct CreateCheckpointTests {
    ...
    /// A completed create whose restore tree was removed by default cleanup.
    private func completedWithRemovedTree(_ f: Fixture) throws {
        try f.create(f.runner(), variant: "regular")
        #expect(f.fake.removed == ["restore_tree"])
        #expect(try f.load().artifacts.filter { $0.name == "restore_tree" }.allSatisfy { $0.availability == .removed })
        f.fake.resetCalls()
    }

    @Test func restartFromPrepareRegeneratesARemovedRestoreTree() throws {
        // Real run 2026-09-17: the advertised rebuild command was itself refused
        // with "artifact restore_tree needed by patch was removed".
        let f = try Fixture(); defer { f.cleanup() }
        try completedWithRemovedTree(f)
        let resumed = try f.runner(keepArtifacts: true).resume(bundleURL: f.bundle, request: .init(restartFrom: .prepare))
        #expect(f.fake.executed == [.prepare, .patch, .restore, .cfw, .firstBoot, .verification])
        #expect(f.fake.treeSeen[.prepare] == "unrecorded, absent")
        #expect(f.fake.treeSeen[.patch] == "available, present")
        #expect(f.fake.treeSeen[.restore] == "available, present")
        #expect(resumed.overallStatus == .succeeded)
        let tree = resumed.artifacts.filter { $0.name == "restore_tree" }
        #expect(tree.map(\.recordedBy) == [.prepare, .patch])
        #expect(tree.allSatisfy { $0.availability == .available && $0.removedReason == nil })
        // --keep-artifacts on the resume is honoured for the new attempt.
        #expect(f.fake.removed == ["restore_tree"])
        #expect(FileManager.default.fileExists(atPath: f.bundle.appendingPathComponent("Restore").path))
        #expect(resumed.attempts.last?.checks["artifact.restore_tree"]?.contains("regenerated by prepare") == true)
        #expect(try f.load().artifacts == resumed.artifacts)
    }

    @Test func restartFromPrepareWithoutKeepArtifactsRemovesTheTreeAgain() throws {
        let f = try Fixture(); defer { f.cleanup() }
        try completedWithRemovedTree(f)
        let resumed = try f.runner().resume(bundleURL: f.bundle, request: .init(restartFrom: .prepare))
        #expect(f.fake.treeSeen[.patch] == "available, present")
        #expect(f.fake.removed == ["restore_tree", "restore_tree"])
        #expect(resumed.artifacts.filter { $0.name == "restore_tree" }.allSatisfy { $0.availability == .removed })
    }

    @Test(arguments: [VPhoneCreateStage.patch, .restore, .cfw, .firstBoot, .verification])
    func restartAfterPrepareWithRemovedTreeIsRefusedWithoutWriting(restart: VPhoneCreateStage) throws {
        let f = try Fixture(); defer { f.cleanup() }
        try completedWithRemovedTree(f)
        let before = f.checkpointBytes()
        do {
            try f.runner().resume(bundleURL: f.bundle, request: .init(restartFrom: restart))
            Issue.record("restart from \(restart.rawValue) accepted a removed restore tree")
        } catch let VPhoneCreateRunError.artifactUnavailable(name, neededBy, rebuild) {
            #expect(name == "restore_tree")
            #expect(neededBy == restart)
            #expect(rebuild == "vphone-cli vm create --resume vm --restart-from prepare")
        }
        #expect(f.fake.executed.isEmpty)
        #expect(f.checkpointBytes() == before)
    }

    @Test func removedArtifactStillRefusedWhenTheRegeneratingStageRunsAfterAConsumer() throws {
        // A stage that both rewrites and consumes an artifact needs the old copy.
        let f = try Fixture(); defer { f.cleanup() }
        try completedWithRemovedTree(f)
        var checkpoint = try f.load()
        let index = checkpoint.artifacts.firstIndex { $0.name == "disk" && $0.recordedBy == .restore }!
        checkpoint.artifacts[index].availability = .removed
        checkpoint.artifacts[index].retainUntil = [.cfw]
        try withExtendedLifetime(VPhoneCreateCheckpointStore.open(bundleURL: f.bundle)) { try $0.commit(checkpoint) }
        #expect { try f.runner().resume(bundleURL: f.bundle, request: .init(restartFrom: .cfw)) } throws: { error in
            guard case let VPhoneCreateRunError.artifactUnavailable(name, neededBy, _) = error else { return false }
            return name == "disk" && neededBy == .cfw
        }
        #expect(f.fake.executed.isEmpty)
    }

    @Test func keptStageArtifactsAreStillValidatedWhenARemovedArtifactIsRegenerated() throws {
        let f = try Fixture(); defer { f.cleanup() }
        try f.create(f.runner(keepArtifacts: true), variant: "regular")
        var checkpoint = try f.load()
        for index in checkpoint.artifacts.indices where checkpoint.artifacts[index].name == "disk" {
            checkpoint.artifacts[index].availability = .removed
            checkpoint.artifacts[index].retainUntil = [.cfw]
        }
        try withExtendedLifetime(VPhoneCreateCheckpointStore.open(bundleURL: f.bundle)) { try $0.commit(checkpoint) }
        let original = try Data(contentsOf: f.bundle.appendingPathComponent("out-patch.txt"))
        try Data("tampered".utf8).write(to: f.bundle.appendingPathComponent("out-patch.txt"))
        f.fake.resetCalls()
        let before = f.checkpointBytes()
        do {
            try f.runner().resume(bundleURL: f.bundle, request: .init(restartFrom: .restore))
            Issue.record("resume accepted a changed artifact of a kept stage")
        } catch let VPhoneCreateRunError.artifactChanged(name, recordedBy, _) {
            #expect(name == "patch" && recordedBy == .patch)
        }
        #expect(f.fake.executed.isEmpty)
        #expect(f.checkpointBytes() == before)

        // With the kept artifact intact, the removed disk is regenerated by restore before cfw needs it.
        try original.write(to: f.bundle.appendingPathComponent("out-patch.txt"))
        let resumed = try f.runner(keepArtifacts: true).resume(bundleURL: f.bundle, request: .init(restartFrom: .restore))
        #expect(f.fake.executed.first == .restore)
        #expect(resumed.attempts.last?.checks["artifact.patch"] == "matches patch")
        #expect(resumed.attempts.last?.checks["artifact.disk"] == "removed; regenerated by restore before cfw")
    }

    @Test func artifactLookupReturnsTheLatestRecord() {
        var checkpoint = VPhoneCreateCheckpoint(
            identity: .init(name: "vm", path: "/vm", directoryId: "1:2"), options: Fixture.options("jb"),
            tool: .init(executableSha256: nil, stageContractVersion: 1), now: Date())
        checkpoint.artifacts = [
            .init(name: "restore_tree", relativePath: "Restore", kind: .treeMetadata, fingerprint: "old", sizeBytes: 1,
                  availability: .removed, recordedBy: .prepare, retainUntil: [.patch]),
            .init(name: "restore_tree", relativePath: "Restore", kind: .treeMetadata, fingerprint: "new", sizeBytes: 1,
                  availability: .available, recordedBy: .patch, retainUntil: [.restore]),
        ]
        #expect(checkpoint.artifact("restore_tree")?.fingerprint == "new")
        #expect(checkpoint.artifact("disk") == nil)
    }
