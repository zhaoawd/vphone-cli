...
struct KernelGateStructuredMergeTests {
...
extension KernelJBStructuredTests {
    @Test(arguments: [1, 2])
    func requiredVmProtectDoesNotHideAmbiguity(candidates: Int) throws {
        ...
        let step = try #require(patcher.buildSteps().first { $0.id.method == "patchVmMapProtect" })
        #expect(step.requirement == .required)
        ...
        let outcome = PatchOutcomeMapping.outcome(for: raw, requirement: .required, gates: Self.gates())
