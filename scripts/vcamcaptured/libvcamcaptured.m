...
#define VCC_OBSERVED_SIZE 128

_Static_assert(sizeof(vcc_shm_header_t) == VCC_SHM_HEADER_SIZE,
               "vcam shm header size drift");
_Static_assert(sizeof(vcc_observed_t) == VCC_OBSERVED_SIZE,
...
// detector's *input* luma with our real QR pixels.  Crop to the dark-pixel QR
// bbox, restore a four-module-equivalent quiet zone, then fill the detector
// frame's short edge.  For the observed 720x1280 input this leaves the dark QR
// matrix at about 80% (575px), centered, without the landscape source's large
// white side margins.  We hold the true payload already; this only ensures the
...
static BOOL vcc_overwrite_luma_from_latest(CMSampleBufferRef sampleBuffer) {
  ...
  if (vcc_latest_frame.pixels &&
    ...
    const uint8_t *src = vcc_latest_frame.pixels;
    size_t minX = srcW, minY = srcH, maxX = 0, maxY = 0;
    BOOL foundDark = NO;
    for (uint32_t y = 0; y < srcH; y++) {
      const uint8_t *srow = src + (size_t)y * srcBpr;
      for (uint32_t x = 0; x < srcW; x++) {
        unsigned b = srow[(size_t)x * 4 + 0];
        unsigned g = srow[(size_t)x * 4 + 1];
        unsigned r = srow[(size_t)x * 4 + 2];
        uint8_t luma = (uint8_t)(16 +
    ...
    // White video-range baseline is also the neutral-generation behavior:
    // no dark bbox means no old QR survives in detector luma.
    for (size_t y = 0; y < dstH; y++) {
      memset(dstY + y * dstYStride, 235, dstW);
    }

    if (foundDark) {
      size_t bboxW = maxX - minX + 1;
      size_t bboxH = maxY - minY + 1;
      size_t darkSide = bboxW > bboxH ? bboxW : bboxH;
      // +12.5% per side reproduces a four-module quiet zone for the
      // live-qualified QR geometry: darkSide / (darkSide + 2*quiet) ~= 80%.
      size_t quiet = (darkSide + 7) / 8;
      size_t cropSide = darkSide + quiet * 2;
      int64_t centerX = ((int64_t)minX + (int64_t)maxX) / 2;
      int64_t centerY = ((int64_t)minY + (int64_t)maxY) / 2;
      int64_t cropX = centerX - (int64_t)cropSide / 2;
      int64_t cropY = centerY - (int64_t)cropSide / 2;
      size_t drawSide = dstW < dstH ? dstW : dstH;
      size_t offX = (dstW - drawSide) / 2;
      size_t offY = (dstH - drawSide) / 2;

      for (size_t y = 0; y < drawSide; y++) {
        int64_t srcY = cropY +
            (int64_t)((y * cropSide + drawSide / 2) / drawSide);
        if (srcY < 0 || srcY >= (int64_t)srcH) continue;
        const uint8_t *srow = src + (size_t)srcY * srcBpr;
        uint8_t *drow = dstY + (offY + y) * dstYStride + offX;
        for (size_t x = 0; x < drawSide; x++) {
          int64_t srcX = cropX +
              (int64_t)((x * cropSide + drawSide / 2) / drawSide);
          if (srcX < 0 || srcX >= (int64_t)srcW) continue;
          unsigned b = srow[(size_t)srcX * 4 + 0];
          unsigned g = srow[(size_t)srcX * 4 + 1];
          unsigned r = srow[(size_t)srcX * 4 + 2];
          drow[x] = (uint8_t)(16 +
              ((66 * r + 129 * g + 25 * b + 128) >> 8));
        }
      }
...
static uint8_t *vcc_observed_base = NULL;
...
static void vcc_copy_bounded(char *dst, size_t capacity, const char *src) {
...
static int vcc_observed_map(void) {
  int fd = open(VCC_OBSERVED_PATH, O_RDWR | O_CREAT, 0644);
  if (fd < 0) {
    vcc_log(@"  observed open(%s) failed: %s",
            VCC_OBSERVED_PATH, strerror(errno));
    return -1;
  }
  if (ftruncate(fd, VCC_OBSERVED_SIZE) < 0) {
    vcc_log(@"  observed ftruncate failed: %s", strerror(errno));
    close(fd);
    return -1;
  }
  void *base = mmap(NULL, VCC_OBSERVED_SIZE, PROT_READ | PROT_WRITE,
                    MAP_SHARED, fd, 0);
  close(fd);
  if (base == MAP_FAILED) {
    vcc_log(@"  observed mmap failed: %s", strerror(errno));
    return -1;
  }
  memset(base, 0, VCC_OBSERVED_SIZE);
  vcc_observed_base = (uint8_t *)base;
  vcc_log(@"  observed receipt mapped %s -> %p",
          VCC_OBSERVED_PATH, vcc_observed_base);
  return 0;
}

static void vcc_publish_observed(uint32_t protocol_version,
                                 const char *generation, const char *role,
                                 uint64_t frame_index) {
  if (!vcc_observed_base) return;
  vcc_observed_t *receipt = (vcc_observed_t *)vcc_observed_base;
  uint64_t prev_seq = atomic_load_explicit(
      (_Atomic uint64_t *)&receipt->seq, memory_order_acquire);
  uint64_t writing_seq = (prev_seq | 1ull) + 2ull;
  atomic_store_explicit((_Atomic uint64_t *)&receipt->seq, writing_seq,
                        memory_order_release);
  receipt->protocol_version = protocol_version;
  receipt->frame_index = frame_index;
  receipt->observed_at_ns = vcc_monotonic_ns();
  vcc_copy_bounded(receipt->generation, sizeof(receipt->generation), generation);
  vcc_copy_bounded(receipt->role, sizeof(receipt->role), role);
  atomic_store_explicit((_Atomic uint64_t *)&receipt->seq, writing_seq + 1ull,
                        memory_order_release);
}
...
static int vcc_shm_read_latest(void) {
  ...
  uint32_t pv  = hdr->protocol_version;
  uint64_t ts  = hdr->timestamp_ns;
  uint64_t idx = hdr->frame_index;
  uint64_t published_at = hdr->published_at_ns;
  uint32_t pix_len = hdr->pixels_length;
  char generation[VCC_GENERATION_MAX] = {0};
  char role[VCC_ROLE_MAX] = {0};
  memcpy(generation, hdr->generation, sizeof(generation));
  memcpy(role, hdr->role, sizeof(role));

  if (pix_len == 0 || pix_len > VCC_SHM_MAX_PIXELS) return 0;
  if (w == 0 || h == 0 || bpr == 0 || pix_len < (size_t)bpr * h) return 0;

  // Stage pixels privately first.  Do not expose the new data to any camera
  // consumer until the second seq read proves that this copy was not torn by
  // a concurrent vphoned publish.
  uint8_t *stable_pixels = (uint8_t *)malloc(pix_len);
  if (!stable_pixels) return 0;
  memcpy(stable_pixels, vcc_shm_base + VCC_SHM_HEADER_SIZE, pix_len);

  // The writer sets an odd seq before it touches the header or pixels and
  // returns it to even only after both writes.  A matching even seq makes the
  // staged copy safe to publish and eligible for a generation receipt.
  ...
  // All consumers hold this mutex while accessing pixels, so replacing the
  // backing allocation makes the validated frame visible atomically with its
  // metadata.  The old allocation cannot be in use after a consumer unlocks.
  pthread_mutex_lock(&vcc_latest_frame.lock);
  uint8_t *old_pixels = vcc_latest_frame.pixels;
  vcc_latest_frame.pixels = stable_pixels;
  vcc_latest_frame.pixels_capacity = pix_len;
  ...
  if (pv == VCC_PROTOCOL_VERSION && generation[0] != '\0' &&
      idx > 0 && published_at > 0) {
    vcc_publish_observed(pv, generation, role, idx);
  }
...
static void vcc_start_frame_receiver(void) {
  ...
  if (vcc_observed_map() < 0) {
    vcc_log(@"  generation receipt disabled (observe mmap unavailable)");
  }
