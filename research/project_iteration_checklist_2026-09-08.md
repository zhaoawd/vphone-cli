...
2026-09-17 晚间状态：**D4、D5 已完成，累计 25/28 项完成，3 项未完成：F1、F3、F4。** D4 在 `8b6365f` 构建上完成 restore、cfw、patch（publishing 中断及事务已提交未写检查点）和 first_boot/verification 断开的真实中断续跑，见 [D4 记录](d4_create_checkpoint_resume_2026-09-17.md)。D5 以签名应用运行，并在本构建启动的 VM 上确认客户机断开/连接报告，见 [D5 记录](d5_diagnostics_2026-09-17.md)。验收中发现 restore 桥接进程在 FCS 密钥请求 SSL 失败后挂起、vm create 无限等待；桥接脚本已增加 URL 资源超时重试与后台任务失败退出，实机注入验收通过，见 D4 记录。F1 方案草稿见 [F1 方案](f1_e2e_matrix_plan_2026-09-17.md)。
