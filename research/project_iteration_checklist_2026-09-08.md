...
2026-09-18 收尾状态：**F4 以“文档与证据一致”的范围关闭，累计 28/28 项完成。** 核对记录见 [F4 交付核对](f4_delivery_verification_2026-09-18.md)。清单此前“远端工作流未运行”的表述已修正：`checks` 有实际运行记录且自 2026-09-12 起连续 7 次失败，`release` 在本仓库无运行记录；失败诊断见 [F4-D CI 失败诊断](f4_ci_failure_diagnosis_2026-09-18.md)；三项已在提交 `2b93671` 修复（含 `VPhoneProcessRunner` 的超时产品缺陷），本机验证通过，改法未在 CI 上验证。2026-09-19 更新：含该修复的运行 `35364546072` 仍失败，新暴露同类别的两项（`testPauseUsesIndependentDeliverySequence`、`test_gesture_pacing_stays_outside_the_recorded_latency`），已修复并在本机与负载下验证，诊断见同一文档第 6 节；同样未在 CI 上验证。28 项之外单独跟踪的未解决问题：远端 CI 失败、F1 的 O1–O3、F3 的遗留项。

2026-09-18 晚间状态：**F3 按用户决定以“本轮实验范围”关闭，累计 27/28 项完成，1 项未完成：F4。** F3 交付命令延迟基线与可复跑基准工具；E2 连接恢复与 E6a–E6c 相机实验未执行，遗留项见 [F3 方案 10.12](f3_performance_baseline_plan_2026-09-18.md)。F1 的未解决问题 O1–O3 仍单独跟踪，不计为通过。

2026-09-18 状态：**F1 按用户决定 B 以“本轮组合证据矩阵”范围关闭，累计 26/28 项完成，2 项未完成：F3、F4。** 最终矩阵见 [F1 支持矩阵](f1_support_matrix_2026-09-17.md)，范围为 P（26.1/23B85）less/regular/dev/jb/exp 与 N（26.6.1/23G82 + cloudOS 26.4）jb/exp --frida；L（旧版本）未纳入。已知限制 L1–L3（regular/less 开发者模式无法开启导致 DDI 阻塞；exp 系统相机 QR 识别提示未观察到；定位应用层、DeviceTree 型号字段与计算路径无探针）不再复测；未解决问题 O1–O3（regular/dev/less uiopen 缺失导致应用启动失败，修正 `5fd007c` 未部署；jb 底部边缘回主屏手势失败；Frida Stalker 卸载与会话分离挂起）单独跟踪，不计为通过，见 [限制与问题配置](f1_known_limits_2026-09-17.json)。F3 方案草稿见 [F3 方案](f3_performance_baseline_plan_2026-09-18.md)。

2026-09-17 晚间状态：**D4、D5 已完成，累计 25/28 项完成，3 项未完成：F1、F3、F4。** D4 在 `8b6365f` 构建上完成 restore、cfw、patch（publishing 中断及事务已提交未写检查点）和 first_boot/verification 断开的真实中断续跑，见 [D4 记录](d4_create_checkpoint_resume_2026-09-17.md)。D5 以签名应用运行，并在本构建启动的 VM 上确认客户机断开/连接报告，见 [D5 记录](d5_diagnostics_2026-09-17.md)。验收中发现 restore 桥接进程在 FCS 密钥请求 SSL 失败后挂起、vm create 无限等待；桥接脚本已增加 URL 资源超时重试与后台任务失败退出，实机注入验收通过，见 D4 记录。F1 方案草稿见 [F1 方案](f1_e2e_matrix_plan_2026-09-17.md)。 F1 进展：P 组合（26.1/23B85）五个变体已创建并完成自动步骤，GUI 首次设置、GUI 输入、DDI 等人工步骤待执行，N（Frida）未开始，L（旧版本）按用户决定不纳入，见 [P 组合运行记录](f1_p_matrix_run_2026-09-17.md)。 同日完成 P 组合人工步骤与 N（Frida）组合 jb/exp 全流程；F1 仍未完成，阻塞与失败项见该记录“F1 当前结论”。

2026-09-17 最新状态：**C5、D3 已完成，累计 23/28 项完成，5 项未完成：D4、D5、F1、F3、F4。** C5 补丁实验记录与真实输入比较见 [C5 记录](c5_patch_experiment_record_2026-09-17.md)。 26.1 四变体迁移前后同输入配对安装及 System/xART/Preboot 三卷对照完成，差异均归因于签名、事件日志、quarantine 时间和 VM 目录路径，见 [D3 真实安装与对照记录](d3_cfw_real_install_2026-09-17.md)。同日较早完成 C4；C4 的完整 less 流水线、事务提交、Manifest 与备份校验通过，见 [C4 记录](firmware_recovery_c4_2026-09-11.md)。此前 F2 的双 VM 验收范围保持不变，见 [F2 记录](f2_dual_vm_acceptance_2026-09-17.md)。下文较早状态保留为历史记录。

2026-09-16 推进：M0 定位修复及既有验收记录已整理提交，无固件回归通过。用户已完成 rig2 首次设置；已取得相机消费回执，并验证图像切换、中性画面、服务及完整操作系统重启后的恢复。QR 标准回调、负向对照、暂停消费拒绝及恢复通过。A3 已增加锁屏/熄屏前置检查，原生路径在亮屏解锁后仍未通过回主屏手势，原因未查明。视频 VM 显示验收受菜单目标定位限制。A3/E6 不关闭。见 [重启与输入最新对照](e6_reboot_a3_input_2026-09-16.md)、[QR 验收](camera_e6_qr_acceptance_2026-09-16.md)及 [A3 历史记录](a3_rig2_2026-09-16.md)。完成计数仍为 17/28。
...
| --- | --- | --- | --- | --- |
| D3 | 已完成（2026-09-17），见 [真实安装与对照](d3_cfw_real_install_2026-09-17.md) | A2、B1 已完成 | 先核对 D1/D2 当前未提交改动，复用共同 Python 环境管理；实际安装前准备各变体输入、独立测试磁盘、空间及挂载权限，确认 VM 已关闭且可取得锁 | 各变体阶段顺序、重复安装、失败清理及迁移前后调用/产物比较通过 |
...
| A3 | 已完成（2026-09-16），见 [触控验收](a3_touch_acceptance_2026-09-16.md) | A1 已完成 | 已验收 rig2 26.1 / 23B85 EXP；其他版本和变体未验证 | 已复现底部手势问题修复；输入与连接矩阵通过，原生系统手势限制已记录 |
...
| C5 | 已完成（2026-09-17），见 [C5 记录](c5_patch_experiment_record_2026-09-17.md) | C1–C4 已完成 | 取得成功和失败运行的输入/输出、工具版本、代码状态及必要补丁结果；安排同输入重复运行和不同选项运行 | JSON 与人类可读摘要来自同一数据；成功/失败记录完整且可比较 |
| D4 | 已完成（2026-09-17），真实 restore/CFW/patch/启动中断续跑通过，见 [D4 记录](d4_create_checkpoint_resume_2026-09-17.md) | B4、C4、D3 已完成 | 确定事务与安装阶段约定；准备可替换阶段执行器；真实恢复实验需专用 VM、可恢复输入、空间及设备状态探测 | 各阶段失败、取消和重启可续跑；输入变化时拒绝错误复用；真实 restore 中断后重新探测状态 |
| D5 | 已完成（2026-09-17），签名应用与客户机连接实测见 [D5 记录](d5_diagnostics_2026-09-17.md) | B2、C5、D2、D4 已完成 | 依赖报告、实验记录和创建状态模型稳定；准备组件缺失及状态损坏样本 | 只读文本/JSON 能区分环境、依赖、输入、补丁、恢复及客户机运行问题 |
| F1 | 可整理精确组合矩阵，并复用已完成的 less 局部证据 | A3、C3、D4、E5、E6 已完成 | 准备主要和旧版本组合、各变体输入及适用工具；现有 less VM 可继续做运行检查，重做恢复须重新生成已删除的成功 AEA | 逐组合完成创建至应用能力的端到端验收；EXP 和 Frida 按适用范围单独验证 |
| F2 | 已完成（2026-09-17），见 [双 VM 实机验收](f2_dual_vm_acceptance_2026-09-17.md) | B4、E3–E6 已完成 | 两实例终态运行；应用、文件、相机、生命周期和离线占用由本次流程覆盖，定位使用 E5 双 VM 证据 | 两实例范围内未发生状态串用或相互清理；限制已单独记录 |
| F3 | 已完成（限定范围，2026-09-18），见 [F3 方案 10.12](f3_performance_baseline_plan_2026-09-18.md) | F2 已完成；F1 未完成 | 固定宿主、固件、CPU/内存、负载、样本数和运行时长；安排连续测试时间 | 记录延迟、资源占用、恢复、相机丢帧及长时运行结果，形成可复跑基线 |
...
### A3 — 验证客户机触控优先策略【已完成；2026-09-16】

最终结果：宿主传递按下时的边缘判定，客户机在整段手势和断线释放中保留 HID 边缘标记。真实连接状态矩阵、点击、列表拖动、长按、三档窗口缩放、按下中断线/重连及纯生产构建复验通过；其他精确组合明确为未验证。无固件回归为 113 项 Python、141 项 XCTest（3 项跳过）和 336 项 Swift Testing，0 失败。原生点击/滚动回退保留，本组合原生底部 Home 手势不支持。详见 [验收证据、部署备份和支持边界](a3_touch_acceptance_2026-09-16.md)。下列追加内容为修复前定位记录。

2026-09-16 追加：rig2 26.1 EXP 在亮屏解锁下的底部上滑重复失败，单独改变起点与时长仍失败；同一截图判据下 Home 键返回主屏通过。主屏点击设置及长按设置快捷菜单取得证据。新增可重复验收工具，A3 不关闭，详见 [底部上滑与 Home 对照](a3_home_navigation_2026-09-16.md)。

同日后续：系统手势已启用、坐标送达，但原事件的边缘类型为 0，应用切换识别器在边缘类型检查处失败。临时忽略 HID 边缘检查后两次返回主屏，撤销后再次失败。隔离事件添加 FromEdgeTip 可进入应用切换器，尚未完成生产输入修复。见 [HID 边缘诊断](a3_hid_edge_diagnosis_2026-09-16.md)。

涉及：`VPhoneControl.swift`、`VPhoneVirtualMachineView.swift`、`scripts/vphoned/vphoned_hid.h`、`vphoned_hid.m`。

- [x] 验证连接状态与 `touch` 能力的选择矩阵：未连接、能力缺失、连接且支持、断线、重新连接。
- [x] 在已准备的旧版 iOS、26.x、27.x 精确组合上检查点击、拖动、长按、边缘坐标、窗口缩放以及按下过程中断线的释放行为；缺少组合时记录未验证。实际仅验收上述 26.1 EXP 组合。
- [x] 检查当前单指路径与原生路径的行为差异，保留有效回退；记录 GUI 证据并独立提交现有触控改动。
...
### C4 — 避免失败留下无法判断的部分修改固件【本地验收完成；依赖 C3、B4】
...
- [x] 在暂存区完成修改和校验，验证通过后使用可恢复的提交记录交付产物。多文件替换不宣称具备文件系统级原子性。
- [x] 在读取、打补丁、保存、Manifest 更新和提交各阶段注入失败；验证磁盘不足、进程退出和重复执行后的恢复行为。
...
2026-09-17：重新生成隔离输入后，完整 less 生产路径首次运行发现提交前 Manifest 校验与生成规则不一致。修复并通过定向回归后，显式恢复失败事务、重跑完整路径成功；XCTest 核对正式 Manifest、备份摘要、提交归档及事务清理。C4 本地验收完成，当前总数见本文开头。

验收：失败前后的原始输入、暂存结果和正式产物均可识别；中断后能够恢复一致状态，不依赖手工猜测。

### C5 — 输出可重现的补丁实验记录【已完成；2026-09-17；依赖 C1–C4】

涉及：固件 CLI、PatchRecord 导出、VPhoneCore 报告模块；拟新增补丁实验记录文档。

- [x] 自动记录输入/输出哈希、项目提交与未提交状态、工具版本、两个固件构建号、变体、选项和必要补丁结果。
- [x] 从同一数据生成 JSON 与人类可读摘要；失败时同样保存已完成部分和失败阶段。
- [x] 对两次完全相同的运行验证记录可比较，对不同选项验证差异可定位。

验收：研究者可以根据记录重建输入条件并判断产物完整性；历史总数不作为当前运行的替代证据。

2026-09-17：`fw patch` 与 `patch-firmware` 增加 `--record-out`，`fw record show/compare` 从同一 JSON 生成摘要与三项比较（实验条件、补丁结果、产物摘要）。13 项专项测试及完整 Swift 回归（Swift Testing 350、XCTest 143/3 跳过）通过。真实 26.1 / 23B85 regular 输入两次相同运行三项比较均相同；`--force-exc-guard` 运行只报告该选项为条件差异；对已补丁输入的运行在 kernelcache 必要补丁失败，记录保存失败阶段、已完成组件和未提交事务，产物标为 unavailable。less 需要 sudo，未做真实运行；固件来源因 `fw prepare` 未保存而记为未知；源码状态在运行时读取，可能与构建时不同。见 [C5 记录](c5_patch_experiment_record_2026-09-17.md)。
...
### D3 — 提取共同 CFW 安装步骤【已完成；2026-09-17；依赖 A2、B1】
...
- [x] 验证每个变体的阶段序列、重复运行和故障清理，比较迁移前后的调用及产物。
...
2026-09-17：四台关机专用 VM 的 regular、dev、JB、EXP 首次及重复 CFW 安装均返回 0；只读清单确认 Cryptex、基础程序与变体文件，镜像挂载和磁盘句柄已清理。regular 重复安装的可执行文件差异仅位于代码签名区域。见 [真实安装记录](d3_cfw_real_install_2026-09-17.md)。迁移前脚本没有对同一真实磁盘输入执行安装，故上方“迁移前后产物比较”仍未全部满足。

2026-09-17 续跑：当前主机没有上一主机受忽略的配对磁盘、运行副本和日志，可用空间 38 GiB，低于批次的 80 GiB 前置阈值；未修改现有 VM。卷清单工具新增可复用 `preflight` 检查，并增加 5 项测试；完整 Python 126 项在允许本地 Unix socket 的环境通过。实际八次安装和三卷产物对照仍未执行，D3 保持进行中。

2026-09-17 对照结果：原主机的八次配对安装全部完成，三卷只读扫描显示四个变体 legacy/current 目录项数量一致，没有仅存在于一侧的文件。差异为 `.fseventsd` 日志、thin/fat Mach-O 签名区域、dev/EXP 中 131 个 AppleDouble 文件的 quarantine 时间字段，以及 JB/EXP `TweakLoader.dylib` 中由配对 VM 目录路径决定的 install name 和 UUID。阶段语句比较、故障清理回归（09-14）与重复安装（09-17）此前已完成，D3 关闭。故障清理仅有无 VM 回归证据，未在真实磁盘上注入故障；客户机启动与首启脚本不在 D3 验收范围内。`preflight` 目录名已对齐为 `vm-<variant>-<side>`。

### D4 — 为创建流程增加检查点与续跑【已完成；2026-09-17；依赖 B4、C4、D3】

涉及：`VPhoneCreateOrchestrator.swift`、创建 CLI、`VPhoneManagedProcess.swift`；拟新增创建状态模型和编排测试。

- [x] 持久保存 prepare、patch、restore、CFW、first-boot、JB-finalize、verification 的状态及输入/产物摘要。
- [x] 增加明确的续跑入口；重新验证前置产物后跳过已完成阶段。版本、选项或产物发生不兼容变化时拒绝盲目续跑。
- [x] 使用可替换的阶段执行器测试每阶段失败/取消/重启；真实 restore 中断须重新探测设备状态，不能仅凭旧检查点继续写入。

验收：同名创建中断后有明确恢复路径；重新执行不会将部分成功误判为整体成功。默认清理策略保留恢复所需产物。

2026-09-17：`vm create` 改由可替换阶段执行器、只读验证器和实时状态探测器驱动，检查点保存在 `<bundle>/.create-checkpoint`；新增 `vm create --resume`、`--restart-from`、`--accept-tool-change` 与只读 `vm create-status [--json]`。53 个测试函数（96 个用例）覆盖设计草案无 VM 表格的全部场景；合并后完整 Swift 回归（Swift Testing 403、XCTest 143/3 跳过）通过。第三项的替身执行器部分已完成，真实 restore、CFW、patch 和启动阶段中断续跑未执行，D4 不关闭。JB finalize 无宿主侧证据采集，始终记为 unverified。见 [D4 记录](d4_create_checkpoint_resume_2026-09-17.md)。

2026-09-17 真实验收（`8b6365f`，regular 26.1 / 23B85，专用 `d4-acc`）：主进程被杀后子进程持锁时续跑拒绝且检查点不变，停止子进程后探测为空闲并从 DFU 重跑 restore；CFW 挂载期间中断时 root 安装进程持锁导致拒绝，其退出后无挂载残留，cfw 重复安装并完成；patch 在 journal `publishing` 时中断提示 `fw patch --recover`，恢复后续跑，事务已提交而检查点未写入时以产物指纹变化拒绝，`--restart-from prepare` 完成；first_boot 与 verification 启动子进程被杀后阶段失败、restore 树保留，续跑只重跑该阶段并最终 `succeeded`。未覆盖：锁已释放但桥接进程/设备端点残留时的探测拒绝、启动阶段主进程被杀、regular 以外变体。restore 桥接进程挂起问题未修复。D4 完成。

### D5 — 统一环境与 VM 诊断输出【已完成；2026-09-17；依赖 B2、C5、D2、D4】

涉及：setup/VM CLI、`boot_host_preflight.sh`、资源和 bundle 报告模块。

- [x] 提供单一诊断入口，报告宿主机条件、签名、依赖、资源、占用、创建阶段和最后失败原因。
- [x] 提供文本与 JSON，使用稳定错误类别；检查本身默认只读，修复动作显式区分。
- [x] 诊断报告默认不包含密码、令牌、客户机文件内容或完整用户数据；验证缺少组件和状态损坏的输出。

验收：使用者能区分环境、依赖、输入、补丁、恢复和客户机运行问题。

2026-09-17：新增只读 `vphone-cli doctor [<vm>] [--json] [--patch-record <record>]`，报告 environment、dependency、occupancy、input、patch、restore、guest_runtime、internal 八类 48 个稳定代码，退出码 ok=0、warning=3、unknown=4、error=5；输出前统一脱敏，修复动作只列建议命令。39 项专项测试及完整 Swift 回归（Swift Testing 452、XCTest 143/3 跳过）通过；宿主、vm-2607、构造的损坏 bundle 和 C5 失败记录的只读样例见 [D5 记录](d5_diagnostics_2026-09-17.md)。未用签名应用运行；未在 E2 之后启动的 VM 上验证客户机连接结果；D4 真实验收未完成，D5 不关闭。另发现含 `..` 的 `--config` 路径不被 `VPhoneBootProcessLocator.parsePIDs` 匹配，可能影响 `requireDFUOwner` 与 D4 实时探测，属待验证假设。

2026-09-17 收尾：`8b6365f` 签名应用运行 doctor，`signing_entitlements` ok；以本构建 headless 启动 `d4-acc` 后先报告 `guest_disconnected`（warning），10 秒内报告 `guest_connected`（15 项能力）；D4 完成后创建状态为 `create_succeeded`；对 `vm-2607` 只读运行时 `parsePIDs` 已匹配含 `..` 的路径（`8b6365f` 修复）。GUI 启动与其他变体的客户机状态未实测。D5 完成。
...
- [x] 验证 generation 切换、旧帧积压、生产已启动但客户机未消费、客户机重启、视频结束及停止后的中性画面策略。
- [x] 选定测试应用分别验证客户机收到帧、应用显示帧和 QR 等应用侧结果，三者独立记录。
...
2026-09-16：配套组件在 rig2 取得视频复制回执；客户机发布共享帧出现红蓝变化，系统相机在 4.84 秒内显示跨越两个 2 秒周期的红蓝序列。视频切换到独立 neutral 呈现后取得新回执并显示白色。结合此前 QR 新旧内容、无消费者拒绝和完整客户机重启证据，E6 关闭。传输、显示和 QR 结果仍分别解释。见 [视频与应用验收](camera_e6_video_acceptance_2026-09-16.md)。

## 八、F：运行验收、性能与文档

### F1 — 建立精确组合的端到端验收矩阵【已完成（限定范围）；2026-09-18；依赖 A3、C3、D4、E5、E6】

涉及：C1 清单、拟新增 `tests/runtime` 和相应研究记录。

- [x] 按可获得样本选择一组主要回归组合和一组旧版本组合，逐步覆盖 less/regular/dev/jb/exp；不宣称尚未测试的排列组合已支持。
- [x] 每组合记录创建、恢复、首次/第二次启动、GUI 输入、文件/应用、DDI、定位、相机及适用的 Frida 客户端会话。
- [x] EXP 额外验证身份相关行为与图形/计算路径，确认非 EXP 变体未获得这些改动；失败绑定精确输入、阶段和日志。
...
2026-09-18 关闭（用户决定 B）：以本轮 P、N 两组组合的证据矩阵为范围关闭，旧版本组合 L 未纳入，因此第一条中的“旧版本组合”未满足，按用户决定接受。矩阵由 `scripts/f1_support_matrix.py` 从验收脚本输出、创建检查点与人工结果生成，见 [F1 支持矩阵](f1_support_matrix_2026-09-17.md)。已知限制 L1–L3 不再复测；未解决问题 O1–O3 单独跟踪，矩阵中保持 failed/partial，不宣称支持。运行过程见 [P/N 组合运行记录](f1_p_matrix_run_2026-09-17.md)。

### F2 — 验证两个 VM 同时工作的完整流程【已完成；2026-09-17】

涉及：拟新增多 VM 自动化测试，实例状态和日志。

- [x] 两个 VM 同时启动，分别进行应用、文件、定位和相机操作，验证任务与结果归属。
- [x] 停止或重启其中一个 VM，确认另一个继续工作；验证重名、路径别名、端口配置和后台任务取消。
- [x] 检查重建/导出/安装时的占用保护，以及失败后的 socket、锁、挂载和进程清理。

验收：两个 VM 不发生状态串用或相互清理。该验收只证明两实例场景，不推导任意规模并发能力。

2026-09-16 准备结果：新增双端点 `file-isolation` 和停止后 `survivor` 场景，5 个本地 Unix socket 集成测试及完整 Python 118 项回归通过。路径别名按 socket 设备号和 inode 拒绝；陈旧 `.vphone-runtime.json` 按诊断记录处理，以目录 flock 判断占用。当前没有在不操作主 VM 的条件下可用的第二个实例，因此未执行应用、定位、相机、重启和离线操作实机流程，本项保持未完成。见 [F2 自动化准备](f2_dual_vm_preparation_2026-09-16.md)。

2026-09-17 实机进展：`vm-2607` 与新建 `vm-2607-f2` 同时运行，代理配置生效及新实例客户机重启后，双端点 `file-isolation` 通过。新实例首次设置阻塞定位为当前宿主环境中缺少 `en0` HTTP/HTTPS 代理配置；只补充当前服务的代理字段，没有复制主实例的网络服务 UUID。应用、相机、停止后存活实例及离线占用保护仍未执行，F2 保持未完成。见 [F2 双 VM 实机验收进展](f2_dual_vm_acceptance_2026-09-17.md)。

2026-09-17 收尾结果：应用按 bundle ID/PID 验证双端归属；文件在停止、重启和后台请求取消后复验；相机按 generation、presentation ID、复制回执及跨实例停止拒绝验证；两次停止新实例时主实例继续工作。重复启动在 VM/端口创建前拒绝；export、clone、fw patch 和 CFW 共享锁入口均在写入前拒绝，未留下事务、挂载或失败进程。定位采用 E5 双 VM 证据。完整 Python 121 项通过。F2 完成，累计 **20/28**；限制见 [F2 双 VM 实机验收](f2_dual_vm_acceptance_2026-09-17.md)。

### F3 — 建立性能和长时间运行基线【已完成（限定范围）；2026-09-18；依赖 F1、F2】

涉及：控制、相机和进程观测，独立基准脚本及报告。

- [x] 固定宿主机、固件、CPU/内存配置与负载，记录命令延迟分位数、CPU/内存与磁盘实际占用。范围：d4-acc（regular，26.1/23B85）单实例；命令延迟见 E1（21 类、4036 次请求、0 失败）。**连接恢复（E2）、相机延迟与丢帧（E6a–E6c）未执行**；相机端到端延迟按决定 8 记为不可测。
- [x] 进行固定时长的重复操作与空闲实验，报告样本数、持续时间及资源增长（E5a 60 分钟、E5a 复跑 64 分钟、E5b 60 分钟、E4 负载 65 分钟、headless 专项 81.8 分钟）；宿主进程与 stdout 日志大小按 3.7 采样。客户机日志大小未采样（regular 无 `shell`）；待处理请求无直接计数，只有间接证据。
- [x] 只针对测量确认的问题安排优化 — 不适用：未检出需要优化的持续增长问题。

验收：得到可复跑的基准，不预先承诺性能提升比例；没有测量依据的优化保留为后续候选项。

2026-09-18 收尾：用户决定在 `Disk.img` 增长定位完成后收尾，不再执行 E2、E3、E1x、E6a–E6c。已交付可复跑基准（`scripts/f3_benchmark.py`、`scripts/f3_host_sampler.py`、`scripts/f3_stats.py` 及离线测试）与命令延迟基线；被测对象未检出持续增长，唯一达到"确认"的 `Disk.img` 项定位为冷启动后 19–33 分钟的一次约 76 MiB 离散分配，不是连续增长。本轮另发现并修复与性能无关的注入手势不串行化缺陷（`a6161e2`，在真实客户机上复验：`discard` 由 45% 归零）。工具修正三项：`96d5857`、`77524de`、`799d168`；阶跃检测、距冷启动时间固定项与 VZ 进程指标口径见 `a78630a`。遗留项（76 MiB 写入方未查明、+225 MiB 事件未复现、修复后手势/截图延迟增量未查明、客户机 `f3-bench` 约 161 MiB 未删、客户机"隔空投送"设置未恢复）见 [F3 方案 10.12](f3_performance_baseline_plan_2026-09-18.md)。

### F4 — 完成文档与交付核对【已完成（限定范围）；2026-09-18；依赖相关工作项完成】

涉及：`AGENTS.md`、README 与多语言文档、研究索引、补丁比较文档、CI/发布说明。

- [x] 更新实际模块图、入口、目录、变体、权限与依赖说明，移除失效文件引用；历史研究结论标明适用日期与组合。模块目录、`make` 目标、CLI 子命令与选项、变体映射、平台与依赖说明经逐条对照代码确认一致；六份文档中的仓库内路径 `test -e` 全部存在。修正四份 README 的失效锚点，补齐 zh/ja/ko 缺少的 FAQ 条目，并为变体计数表加口径与日期说明（表内数字来源未查明，未改动）。
- [x] 从兼容性清单和运行记录生成计数/支持范围；记录 headless 能力以及故障恢复命令。四份 README 新增 Support Scope/支持范围与 Recovery/恢复章节；三套计数按来源分列不合并。开发包与完整包的区别为部分记录（变体表与 `AGENTS.md` 的 `make build` / `swift build` 说明），未单列章节。
- [x] 核对全部工作项的提交和验收结果，对仍缺固件/GUI/客户机证据的项保持未完成状态。28 项逐项核对：主验收文档 `test -e` 全部存在，清单写出的 21 个显式提交 hash 全部命中，未发现状态与证据不符的项。

验收：文档命令可复现，支持范围与结果一致；根目录不引入被仓库规则禁止的任务文件。

2026-09-18 收尾：核对记录见 [F4 交付核对](f4_delivery_verification_2026-09-18.md)，文档改动见提交 `6d9dd03`。本项按"文档与证据一致"的范围关闭，累计 **28/28**。以下事项不在 F4 判据内，单独跟踪，不计为通过：

1. **远端 `checks` 自 2026-09-12 起持续失败**（最近 7 次）。这同时修正了本清单此前"远端工作流未运行"的表述：`checks` 有实际运行记录，`release` 在本仓库仍无运行记录。诊断见 [F4-D CI 失败诊断](f4_ci_failure_diagnosis_2026-09-18.md)。三项失败中两项判定为测试缺陷，一项为产品缺陷：`VPhoneProcessRunner` 的超时排入 `DispatchQueue.global()` 并带 `guard target.isRunning`，队列饱和时超时被延后或不生效；传 `timeout` 的调用点只有 `VPhoneDiagnosticChecks` 的 `doctor` 检查。三项均与 F3 批次提交无关，已在提交 `2b93671` 修复并通过本机验证。
2. F1 的未解决问题 O1–O3 与 F3 的遗留项需要 VM 与客户机，见各自记录。
3. 变体计数表 4/42/53/113/141 与 2/10/12/14/18 的来源与适用组合未查明；`0_binary_patch_comparison.md` 的三个汇总表仍无逐格日期与证据链接。
