...
2026-09-16 推进：M0 定位修复及既有验收记录已整理提交，无固件回归通过。用户已完成 rig2 首次设置；已取得相机消费回执，并验证图像切换、中性画面、服务重启及暂停接收后的恢复。QR 在客户机直接检测仍失败，同一共享帧在宿主可解码；应用内底部上滑仍未返回主屏。A3/E6 不关闭。见 [后续验收](camera_e6_followup_2026-09-16.md)及 [A3 本轮记录](a3_rig2_2026-09-16.md)。完成计数仍为 17/28。
