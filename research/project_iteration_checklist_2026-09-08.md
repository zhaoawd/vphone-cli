...
| D4 | 实现与无 VM 测试完成（2026-09-17）；真实 restore/CFW/patch/启动中断验收待执行，见 [D4 记录](d4_create_checkpoint_resume_2026-09-17.md) | B4、C4、D3 已完成 | 确定事务与安装阶段约定；准备可替换阶段执行器；真实恢复实验需专用 VM、可恢复输入、空间及设备状态探测 | 各阶段失败、取消和重启可续跑；输入变化时拒绝错误复用；真实 restore 中断后重新探测状态 |
| D5 | 实现与无 VM 测试完成（2026-09-17），见 [D5 记录](d5_diagnostics_2026-09-17.md)；待 D4 验收及签名应用运行 | B2、C5、D2 已完成；D4 未完成 | 依赖报告、实验记录和创建状态模型稳定；准备组件缺失及状态损坏样本 | 只读文本/JSON 能区分环境、依赖、输入、补丁、恢复及客户机运行问题 |
| F1 | 可整理精确组合矩阵，并复用已完成的 less 局部证据 | A3、C3、E5、E6 已完成；D4 未完成 | 准备主要和旧版本组合、各变体输入及适用工具；现有 less VM 可继续做运行检查，重做恢复须重新生成已删除的成功 AEA | 逐组合完成创建至应用能力的端到端验收；EXP 和 Frida 按适用范围单独验证 |
