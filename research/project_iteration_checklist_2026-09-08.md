...
| D4 | 2026-09-17 开始实现状态模型、阶段执行器与续跑入口；真实 restore 中断验收待实现后执行 | B4、C4、D3 已完成 | 确定事务与安装阶段约定；准备可替换阶段执行器；真实恢复实验需专用 VM、可恢复输入、空间及设备状态探测 | 各阶段失败、取消和重启可续跑；输入变化时拒绝错误复用；真实 restore 中断后重新探测状态 |
