...
2026-09-18 收尾状态：**F4 以“文档与证据一致”的范围关闭，累计 28/28 项完成。** 核对记录见 [F4 交付核对](f4_delivery_verification_2026-09-18.md)。清单此前“远端工作流未运行”的表述已修正：`checks` 有实际运行记录且自 2026-09-12 起连续失败，`release` 在本仓库无运行记录；失败诊断见 [F4-D CI 失败诊断](f4_ci_failure_diagnosis_2026-09-18.md)。三项已在提交 `2b93671` 修复（含 `VPhoneProcessRunner` 的超时产品缺陷）。2026-09-19 的运行 `35364546072` 新暴露两项固定墙钟窗口断言，已在 `557ce11` 修复；随后运行 `35411246357` 的 Python 3.14 作业又暴露宿主采样测试只接受 `ps`、排除合法 `ps+footprint` 记录的问题。本地已用受控延迟稳定复现并修正，见同一诊断文档第 7 节；2026-09-24 本地 `make test` 通过并已提交，尚未在 CI 上验证。28 项之外单独跟踪的未解决问题：远端 CI 失败、F1 的 O1–O3、F3 的遗留项。
