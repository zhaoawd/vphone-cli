...
2026-09-16 最新状态：**A3、E6 已完成，累计 19/28 项完成，9 项未完成。** A3 已修复客户机 HID 边缘标记缺失，rig2 26.1 / 23B85 EXP 的输入矩阵通过，见 [A3 验收与支持范围](a3_touch_acceptance_2026-09-16.md)。E6 已完成 generation/presentation 隔离、未消费拒绝、客户机重启、QR 标准回调、视频跨 EOF 循环、系统相机动态显示和 neutral 策略验收，见 [E6 视频与应用验收](camera_e6_video_acceptance_2026-09-16.md)。F2 已新增双端点文件归属、路径别名拒绝、停止后清理及存活实例检查工具和集成测试；真实双 VM 流程尚未执行，见 [F2 自动化准备](f2_dual_vm_preparation_2026-09-16.md)。其他版本与变体未验证；C4 继续暂停。下文较早状态保留为历史记录。
...
| F2 | 双端点文件归属、路径别名拒绝、停止后清理及存活实例检查工具与测试已完成；待实机全流程 | B4、E3–E6 已完成 | 两个可同时运行的独立 VM、足够宿主资源、无其他任务占用；E5 的定位隔离仅覆盖其中一部分 | 应用、文件、定位、相机、单实例停止/重启及离线占用保护全流程通过 |
