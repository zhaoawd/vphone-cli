...
2026-09-18 收尾状态：**F4 以“文档与证据一致”的范围关闭，累计 28/28 项完成。** 核对记录见 [F4 交付核对](f4_delivery_verification_2026-09-18.md)。清单此前“远端工作流未运行”的表述已修正：`checks` 有实际运行记录且自 2026-09-12 起连续 7 次失败，`release` 在本仓库无运行记录；失败诊断见 [F4-D CI 失败诊断](f4_ci_failure_diagnosis_2026-09-18.md)，其中一项为 `VPhoneProcessRunner` 的超时产品缺陷，未修复。28 项之外单独跟踪的未解决问题：远端 CI 失败、F1 的 O1–O3、F3 的遗留项。
