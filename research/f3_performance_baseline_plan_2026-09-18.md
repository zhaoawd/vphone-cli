...
| 距冷启动的时间 | 记录被测 VM 的启动时刻与本次运行开始时该 VM 已运行的时长；同一对比的各次运行必须处于同一冷启动后时间窗 | 运行开始时只读 `<bundle>/.vphone-runtime.json` 的 `pid` 与 `startedAt`，与运行开始时刻一起写入 `run.json` 的 `vm.boot`（`started_at`、`uptime_at_run_start_s`、`pid`、`pid_alive`、`stale`、`unavailable`）；文件缺失或 `pid` 已不存在时记录已知项并标注缺失项，不中止运行 |

固定“距冷启动的时间”的原因：10.11 已定位 `Disk.img` 的增长为冷启动后 19–33 分钟的一次约 76 MiB 离散分配，E5a 与 E5b 的差异由测量窗口是否包含该阶跃决定，不是定位源。该变量不固定时，同一实验设置的两次运行可能落在不同的冷启动后时间窗，斜率不可比。`.vphone-runtime.json` 记录的是宿主进程启动时刻，不是客户机内核启动时刻。
...
手势节流：注入器一次只执行一个手势，手势执行期间到达的请求以 `code=gesture_busy` 被拒绝（`tap` 占用 80 毫秒，`swipe` 占用其 `ms` 时长，见 10.10）。因此三个手势类共用一个最小间隔，从一次预约开始到下一次预约开始计时：`tap` 两类合计不小于 `--tap-spacing-ms`，`swipe` 不小于 `--swipe-spacing-ms`。预约时刻写入样本记录的 `t_paced_ns`，与 `t_start_ns` 同一时钟；请求实际开始时刻不早于预约时刻，二者之差是宿主调度耗时，不受该间隔约束，因此相邻请求的 `t_start_ns` 间隔可以短于该间隔。`--swipe-spacing-ms` 默认由本次运行发送的 `--swipe-ms` 加 50 毫秒余量得出，改变 `ms` 时长间隔随之改变。等待发生在请求开始之前，不计入 `t_total`；轮次中已经消耗在其他命令类上的时间计入该间隔，已超过间隔的轮次不再等待；非手势命令类不受该间隔约束。生效值记入 `run.json` 的 `parameters`。
...
- 内存以 `footprint` 为主指标，RSS 为辅助指标；两者口径不同，不相互替代。该设定适用于 vphone-cli 进程，不适用于 Virtualization XPC 进程（见下一条）。
- 判定对象与背景协变量的区分（依据 10.6、10.9 的实测）：3.6 的增长判定适用于被测 vphone-cli 进程、磁盘文件与日志文件。Virtualization XPC 进程按本表原有频率采样并在报告中列出，但只作背景协变量，不参与增长判定。两条实测理由：其一，该类进程的 `footprint` 在 E5a、E5b、E5a 复跑三次运行中恒为 8290 MiB，等于客户机内存分配值，对数十 MiB 级变化不敏感；其二，其 RSS 由大幅阶跃主导，三次运行的斜率为 +28、+31、−1877 MiB/小时，方向不一致。宿主整体序列与数据卷可用空间按 3.4 同样只作背景记录。
- Virtualization XPC 进程的识别依据宿主采样器自身的可执行文件扫描，与该 pid 由 `--pid` 显式给出还是由扫描发现无关，也不依赖人工标签：采样器在每条进程记录上写 `is_vz`。早于该字段的运行目录回退到同一采样器的 `vz_scan` 记录中的 pid 列表。两者都不存在时口径记为 `unknown`（表中“归属未知”），并写出原因，不按判定对象处理，也不猜测。`summarize` 为每条序列标记 `judged`（“判定对象”）、`covariate`（“背景协变量”）或 `unknown`，写入 `summary.json` 并在 `summary.md` 的斜率表与增长判定表中列出。采样频率、序列构建与斜率估计不因该标记改变。
...
- 阶跃检测（依据 10.11）：对每条参与判定的序列先做逐点差分，用稳健、无量纲的判据识别离散阶跃。差分偏离差分中位数超过 8 倍稳健尺度的点记为阶跃。稳健尺度按两种情形取值：
  1. 差分存在可测离散度时，取差分的中位数绝对偏差与四分位距除以 1.349 两者中的较大者。
  2. 该离散度为 0（四分之三以上的差分取同一值）时，改由移动量给出。此情形覆盖量化序列：`footprint` 以整 MiB 报告，缓慢上升表现为若干次 1 MiB 跳变，按情形 1 会把每个量化台阶读成离散事件。判据为：以最小移动量为候选格点，当序列全部数值都是该格点的整数倍且最小数值不低于 8 个格点时，认定序列在该格点上量化，以格点为尺度；移动量不止一个时，取格点与移动量中位数的较大者，使尺寸相近的连续台阶不被判为事件；只有一个移动量且无量化证据时，该移动量记为阶跃。“最小数值不低于 8 个格点”是必要条件：只有一次跳变的平坦序列，其两个数值必然是跳变量的 0 倍与 1 倍，该条件排除这种无信息的“整数倍”。
  该判据不含绝对数值阈值，对 MiB、KiB 与 CPU 核序列同样适用。只有与净变化同向的阶跃才可能解释该净变化，因此占比按同向阶跃合计计算；反向阶跃抵消净变化的一部分，计入会使安静序列出现大于 1 的占比。同向阶跃合计占净变化的比例超过 0.5 时，该序列记为“阶跃主导”；该比例是判断取值，由 `--step-dominance-share` 给出并随运行写入 `summary.json`。实现见 `scripts/f3_stats.py` 的 `step_summary`。
- 判定规则（事先写定，不设数值阈值）：
  1. 95% 区间下限 > 0 且未被阶跃主导的指标记为“增长候选”，只报告，不据此优化。
  2. 同一实验设置的独立第二次运行中该指标再次满足条件 1，且两次斜率同号，记为“测量确认的增长”，进入第 3 项优化候选评估。
  3. 其余情况（第 4 项除外）记为“未检出持续增长”，并同时报告区间宽度（检测能力有限）。
  4. 区间下限 > 0 的运行全部被阶跃主导时，记为“阶跃主导”（`summary.json` 取值 `step_dominated`），不计入第 1、2 项。事实依据见 10.11：被 3.6 判为“测量确认的增长”的 `Disk.img` 序列由冷启动后 19–33 分钟的一次约 76 MiB 离散分配主导；落在窗口内的阶跃同样使 Theil–Sen 斜率的 bootstrap 区间排除 0，因此第 1、2 项本身不能区分阶跃与趋势。
- 阶跃列表（每个阶跃相对分析窗口起点的时间偏移与大小）、阶跃合计、同向阶跃合计与其占净变化的比例写入 `summary.json`，并在 `summary.md` 的斜率表旁列出。
...
| headless 专项（冷启动后 0 分钟起，5 秒采样） | 81.8 分钟 | +91.1 MiB | t+19.33 分 +75.5 MiB（发生在单个 5 秒间隔内），全程仅此一次 |
...
- E5a 中 t+54 分的 +225 MiB 事件在 E5a 复跑中未出现（该处为 +19 与 +8 MiB）。headless 专项覆盖冷启动后 0–82 分钟，其中包含 E5a 该事件对应的时刻（冷启动后约 78 分钟），未出现第二次不小于 5 MiB 的阶跃。事实：该事件未复现；E5a 为 GUI 启动而本轮为 headless，是否与启动模式相关未确定。
- headless 专项的分段速率（去掉阶跃后）：冷启动后 0–19 分钟 +32.8 MiB/小时，20–45 分钟 +2.8 MiB/小时，45–82 分钟 +7.7 MiB/小时。事实：阶跃之外还存在一个随开机时间下降的慢速分量，量级为每小时数 MiB，与 E5b 的 +4.3 MiB/小时、E4 的 +22.2 MiB（65 分钟）处于同一量级。
- 3.6 的判定规则需要补充阶跃检测：在判定“持续增长”前先检查序列是否由少数离散跳变主导。本节的逐点差分即为可用方法。该改动尚未实施。

### 10.12 F3 收尾（2026-09-18）

用户决定在 `Disk.img` 定位完成后收尾，不再继续 F3 的剩余实验。
...
| 0 | 决定项确认 | 完成（第 9 节） |
| 1 | 工具与测试 | 完成 |
| 2 | 独立工作树构建与签名 | 完成（10.2） |
| 3 | 客户机状态固定 | 完成（10.3） |
| 4 | E0、E1、E5a、E5b、E4 | 完成（10.4–10.8） |
| 5 | E2 冷启动恢复 | **未执行** |
| 6–7 | 新建 exp 与 E1x、E3、E6a、E6b、E6c | **未执行** |
| 8 | 汇总与 R 复跑 | 部分完成：E5a 复跑（10.9）、E1 两次复跑（10.9、10.10） |
| 9 | 结束处理 | 见下 |
| 10 | 优化方案 | 不适用：未产生需要优化的持续增长问题 |

#### 结论

1. 命令延迟基线见 10.4（21 类，4036 次请求，0 失败）与 10.10 的配速重跑。非手势命令类在三次独立运行中一致。
2. 资源增长：被测对象未检出持续增长。唯一达到 3.6“确认”的 `Disk.img` 项经 10.11 定位为冷启动后 19–33 分钟的一次约 76 MiB 离散分配，不是连续增长；停机后镜像实际占用会回落。
3. 本轮发现并修复了一个与性能无关但影响自动化正确性的缺陷：注入手势不串行化导致近半数手势在客户机侧留下未释放触摸，可产生非预期的界面操作（10.10，提交 `a6161e2`）。
4. 工具修正三项：斜率窗口排除负载后空载尾段（`96d5857`）、手势类配速（`77524de`）、tap 坐标（`799d168`，该项后被 10.10 证明不是漂移的成因）。

#### 未完成与遗留

| 项 | 状态 |
| --- | --- |
| 76 MiB 分配事件的写入方 | 未查明。区分客户机写入与宿主稀疏文件分配需要带 `shell` 的客户机 |
| E5a 中 t+54 分的 +225 MiB 事件 | 是否可复现未确定 |
| 3.6 判定规则缺少阶跃检测 | 已实施：3.6 的阶跃检测与判定规则第 4 项，依据 10.11 |
| “距冷启动的时间”列入第 2 节固定项 | 已实施：2.1 固定项一行，运行时记入 `run.json` 的 `vm.boot`；依据 10.11 确认它是 E5a/E5b 差异的实际因素 |
| VZ 进程内存指标口径 | 已定：3.3 将该类进程列为背景协变量，仍采样与报告，但不参与增长判定。理由为 footprint 恒为客户机内存分配值而不敏感、RSS 由大幅阶跃主导 |
| 手势与截图延迟在修复后的增量 | 未查明（10.10），构建与启动会话两个变量同时变化 |
| 客户机 `/var/mobile/Library/f3-bench/` | **未删除**。第 9 节决定 6 授权删除，但删除路径依赖 `shell`，regular 客户机不声明该能力。遗留文件为 `w0-{1024,65536,1048576,load16m,load64m,save16m,save64m}.bin`，合计约 161 MiB |
| 客户机“隔空投送”设置 | 在 10.10 的排查与诊断运行中被误触改动，未恢复 |
| 客户机“自动锁定” | 实验开始前观察值即为“永不”，本轮未修改，无需恢复（10.3） |
| 构建工作树 | `.build/f3/src`（`0d7b009`，基线构建）与 `.build/f3diag/src`（`a6161e2`，含手势修复与日志）保留 |

事实：本轮全部提交在本地分支 `codex/autophone-location-multivm-integration`，未推送。
