...
### 10.10 前台页面漂移的排查（2026-09-18，原因未查明）
...
1. 在注入路径上记录事件流（每个 `mouseDown`/`mouseDragged`/`mouseUp` 的时刻与坐标），再跑一次 E1，用日志判断是否发生手势交叉以及误触发生在哪一步。这是能给出证据的路径。
2. 在视图层串行化手势（新手势等待上一手势的排队事件发完），再跑两次 E1 看现象是否消失。这是按待验证假设直接改，不能独立证明因果。
3. 只改基准：把 `tap` 与 `swipe` 从交替轮次改为独立分段并在段间留停顿，每段结束后校验前台页面。这能保住测量条件，但不解决注入路径本身的问题。
