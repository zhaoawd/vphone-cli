...
2026-09-22 修复：有期限的 `runCapturing` 改为调用线程内轮询两个非阻塞管道及单调时钟，不再依赖全局 Dispatch 队列的定时回调。期限到达时发送 SIGTERM，250 ms 后仍运行则发送 SIGKILL；到期后最多再观察 1 秒，随后关闭捕获管道。期限覆盖子进程退出和输出回收，因此直接子进程已退出、后代仍持有管道时也会返回 `timedOut=true`。超时结果的 `succeeded` 恒为 false。回归测试覆盖忽略 SIGTERM、后代继承管道及两路大输出。
