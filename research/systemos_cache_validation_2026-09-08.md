...
日期：2026-09-08。对应[迭代清单 A2](/Users/qcz3840/github/vphone-cli/research/project_iteration_checklist_2026-09-08.md)。

## 实现结果

实现和验证已完成，本记录随 A2 独立提交。Regular、DEV、EXP 的 SystemOS 缓存路径统一调用 [cache_systemos.py](/Users/qcz3840/github/vphone-cli/scripts/cache_systemos.py)。JB 继续通过调用 Regular 继承此逻辑。
...
实际修改入口：[Regular](/Users/qcz3840/github/vphone-cli/scripts/cfw_install.sh)、[DEV](/Users/qcz3840/github/vphone-cli/scripts/cfw_install_dev.sh)、[EXP](/Users/qcz3840/github/vphone-cli/scripts/cfw_install_exp.sh)。未调整 AppOS 缓存、挂载目录、补丁内容或触控代码。本次没有新增二进制补丁。

## 已确认的行为

| 对象 | 现象与证据 | 处理 |
| --- | --- | --- |
| 原有非 AEA 分支 | 任意非 `AEA1` 内容都会被复制到正式缓存 | 复制到临时文件并验证后才发布 |
| 原有缓存判断 | 只检查正式缓存文件是否存在 | 复用前校验镜像；不完整缓存重建 |
| 原有输出路径 | `cp`、`aea` 直接写正式缓存 | 独立临时目录与原子替换 |
...
[test_systemos_cache.py](/Users/qcz3840/github/vphone-cli/tests/test_systemos_cache.py) 新增 9 个 unittest，包含多组输入和入口子用例：
...
[systemos_cache_native.py](/Users/qcz3840/github/vphone-cli/tests/systemos_cache_native.py) 为显式运行的 macOS 检查，不加入默认 unittest 发现流程。它需要能够访问 DiskImages 系统服务：
...
只有 `ipsw` 密钥查询由本地测试密钥替代。此结果未验证 Apple 固件 FCS 密钥获取，也未验证实际 iPhone SystemOS 固件或完整 CFW 安装。测试环境同 [A1 基线](/Users/qcz3840/github/vphone-cli/research/test_baseline_2026-09-08.md)，没有使用既有 VM 磁盘。
...
本地日志：[完整测试](/Users/qcz3840/github/vphone-cli/research/artifacts/a2-2026-09-08/test.log)、[原生验证](/Users/qcz3840/github/vphone-cli/research/artifacts/a2-2026-09-08/native.log)、[构建](/Users/qcz3840/github/vphone-cli/research/artifacts/a2-2026-09-08/build.log)。这些日志位于 Git 已忽略的 `research/artifacts`。
