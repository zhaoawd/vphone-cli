...
C4 完整 less 成功路径及失败事务显式恢复已通过，见 [C4 记录](firmware_recovery_c4_2026-09-11.md)。D3 已完成四变体在关机专用 VM 上的首次与重复真实安装，见 [D3 记录](d3_cfw_real_install_2026-09-17.md)；迁移前后同输入的完整磁盘产物对照仍缺失。`VPhoneCreateOrchestrator.run` 当前仍按顺序直接调用各阶段，`vm create` 仍拒绝同名目录，尚无检查点文件或续跑入口。D4 的状态模型、执行器与真实中断恢复仍待实现。
